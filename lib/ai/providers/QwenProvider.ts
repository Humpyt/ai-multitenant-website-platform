...
export class QwenProvider extends LLMProvider {
  ...
  async generateSite(businessData: BusinessData): Promise<GenerationResult> {
    ...
    try {
      ...
      const generatedContent = response.data.output?.choices?.[0]?.message?.content;
      ...
      return {
        ...
        generationTime: Date.now() - startTime,
        error: error instanceof Error ? error.message : 'Qwen API failed',
      };
    }
  }
  ...
  protected parseAIResponse(response: string): GenerationResult {
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to parse Qwen response',
      };
    }
  }
}