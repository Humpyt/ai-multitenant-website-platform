...
export class ZaiProvider extends LLMProvider {
  ...
  async generateSite(businessData: BusinessData): Promise<GenerationResult> {
    ...
    try {
      ...
      const response = await axios.post(
        `${this.baseURL}/chat/completions`,
        {
          ...
          messages: [
            {
              role: 'system',
              content: 'You are an expert web developer and designer specializing in lightning-fast website generation. Always respond with valid JSON containing complete website code. Never include explanations outside the JSON structure.',
            },
            {
              role: 'user',
              content: prompt,
        ...
        },
        {
          ...
          timeout: 60000, // 1 minute timeout - optimized for speed
        }
      );
      ...
      if (!generatedContent) {
        throw new Error('No content received from Z.ai API');
      }
      ...
      return {
        ...
        generationTime: Date.now() - startTime,
        error: error instanceof Error ? error.message : 'Z.ai API failed',
      };
    }
  }
  ...
  protected parseAIResponse(response: string): GenerationResult {
    try {
      ...
      console.error('Error parsing Z.ai response:', error);
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to parse Z.ai response',
      };
    }
  }
}